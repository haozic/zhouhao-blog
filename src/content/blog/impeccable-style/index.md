...
tags:
  ...
  - 前端设计
  - 设计技能
heroImage: { src: './cover.webp', color: '#0d0f14' }
---
...
- 官网：[https://impeccable.style](https://impeccable.style)（官网界面本身也使用这套规则设计）。
...
注意：这些指令可以组合使用，以实现更复杂的优化。例如，`/audit + /polish` 可以先审计问题再进行精修。

![Impeccable.style](./cover-skills.webp)

## 如何使用

- 安装/集成：访问 [https://impeccable.style](https://impeccable.style) 下载 skill 包，并添加到你的 AI 工具（如 Claude 或 Cursor）中。推荐方式：在终端运行 `$ npx skills add pbakaus/impeccable`（自动安装并配置）。
...
- 在 Prompt 中应用：描述你的界面需求后（如"设计一个基于 Tailwind CSS 的登录页面"），在末尾添加指令组合。AI 会根据 Impeccable 的规则自动优化。示例：`设计一个登录页面，使用 Tailwind CSS。/distill + /bolder + /animate`
